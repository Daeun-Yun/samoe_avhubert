...
class AVHubertSeq2SeqConfig(AVHubertAsrConfig):
    ...
    moe_mode: str = field(default='none', metadata={'help': "SA-MoE mode: 'none', 'ffn_only', 'full'"})
...
class HubertEncoderWrapper(FairseqEncoder):
    ...
    def forward(self, source, padding_mask, **kwargs):
        ...
        x, padding_mask, noise_logits = self.w2v_model.extract_finetune(**w2v_args)
        # B x T x C -> T x B x C
        x = x.transpose(0, 1)
        ...
        return {
            ...
            "encoder_padding_mask": padding_mask,  # B x T
            "padding_mask": padding_mask,
            "noise_logits": noise_logits,  # [T, B, 4]
        }
...
class AVHubertSeq2Seq(FairseqEncoderDecoderModel):
    ...
    def build_model(cls, cfg, task):
        ...
        # pretrain 체크포인트에 moe_mode 키가 없을 경우 강제 설정
        from omegaconf import open_dict
        with open_dict(w2v_args.model):
            w2v_args.model.moe_mode = cfg.moe_mode
        ...
        encoder = HubertEncoderWrapper(encoder_) # avhuber 인코더 구조를 불러와.  
        if state is not None and not cfg.no_pretrained_weights:
            # set strict=False because we omit some modules
            del state['model']['mask_emb']
            encoder.w2v_model.load_state_dict(state["model"], strict=False) #PRETRAIN 가중치 로드 
    ...
    def forward(self, **kwargs):
        ...
        if output["noise_logits"] is not None:
            decoder_out[1]["noise_logits"] = output["noise_logits"]  # criterion에서 NLL loss 계산용
        return decoder_out
