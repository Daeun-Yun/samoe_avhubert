...
class AVHubertDataset(FairseqDataset):
    ...
    def load_feature(self, mix_name, index=None):
        """
        ...
        if 'audio' in self.modalities:
            ...
            if np.random.rand() < self.noise_prob:
                wav_data = self.add_noise(wav_data, index)
            else:
                self._noise_label = 0  # 노이즈 미적용 → 화자 0명
    ...
    def select_noise(self, cur_index=None):
        #인덱스 받아서 자기 자신 인덱스는 제외시키기 (이름 리스트)
        ...
        self._noise_label = num_speakers  # __getitem__에서 읽어 배치 라벨로 사용

        # intervention 0명: add_noise에서 10dB 처리
        if num_speakers == 0:
            return None, 0

        # noise_num에 해당하는 수의 화자 뽑아서 합성하기
        rand_indexes = np.random.choice(indices, size=num_speakers, replace=False)
        noise_wavs = []
        for idx in rand_indexes:
            _, audio_fn = self.names[idx]
            audio_fn = audio_fn.split(':')[0]
            noise_wavs.append(wavfile.read(audio_fn)[1].astype(np.float32))

        # 1명도 normalize해서 반환 (논문: 모든 화자 동일 energy로 normalize)
        min_len = min(len(x) for x in noise_wavs)
        noise_wavs = [x[:min_len] for x in noise_wavs]
        normalized = []
        for wav in noise_wavs:
            rms = np.sqrt(np.mean(np.square(wav)))
            normalized.append(wav / rms if rms > 0 else wav)
        return np.sum(np.stack(normalized), axis=0), num_speakers

    def add_noise(self, clean_wav, cur_index=None):
        clean_wav = clean_wav.astype(np.float32)
        noise_wav, num_speakers = self.select_noise(cur_index)

        #clean+noise synthesis
            #noise 구간 자르기
        ...
        if num_speakers == 0 and self.noise_mode not in (None, 'none', 'None'):
            ...
            _, audio_fn = self.names[idx]
            audio_fn = audio_fn.split(':')[0]
            noise_wav = wavfile.read(audio_fn)[1].astype(np.float32)
            ...
            adjusted_noise_wav = noise_wav * (clean_rms / (10**(10/20)) / noise_rms)
            mixed = clean_wav + adjusted_noise_wav
        elif num_speakers == 0:
            # none 모드에서 0명: 노이즈 없이 clean wav 그대로 반환
            return clean_wav.astype(np.int16)
        elif self.noise_mode == 'mix':
            # mix 모드 intervention 1~3명: 각 화자 개별 에너지 = 타겟 에너지 (total noise = √N × target) #de
            if len(clean_wav) > len(noise_wav):
                ratio = int(np.ceil(len(clean_wav)/len(noise_wav)))
                noise_wav = np.concatenate([noise_wav for _ in range(ratio)])
            if len(clean_wav) < len(noise_wav):
                noise_wav = noise_wav[:len(clean_wav)]
            adjusted_noise_wav = noise_wav * clean_rms #de
            mixed = clean_wav + adjusted_noise_wav
        else:
            # None 모드 intervention 1~3명: 각 화자 개별 에너지 = 타겟 에너지 (total noise = √N × target) #de
            if len(clean_wav) > len(noise_wav):
                ratio = int(np.ceil(len(clean_wav)/len(noise_wav)))
                noise_wav = np.concatenate([noise_wav for _ in range(ratio)])
            if len(clean_wav) < len(noise_wav):
    ...
    def __getitem__(self, index): ##여기다가 모달리티 dropout 넣기!
        video_feats, audio_feats = self.load_feature(self.names[index], index)
        ...
        return {"id": index, 'fid': fid, "video_source": video_feats, 'audio_source': audio_feats, "label_list": labels, "noise_label": getattr(self, '_noise_label', 0)}
    ...
    def collater(self, samples):
        ...
        batch = {
            ...
            "noise_label": torch.LongTensor([s["noise_label"] for s in samples]),
        }
